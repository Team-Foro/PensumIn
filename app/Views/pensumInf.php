<!-- Contenido de la Vista de Estudiante (Informática) -->
...
<form id="actualizar" method="post" action="<?php echo base_url('/pensum/actualizar') ?>">
    <!-- TABLA DEL PENSUM -->
    <table border="1" , bgcolor=666666 class="table table-dark">
        ...
        <?php foreach ($pensum as $semestre => $tiposMaterias) : ?>
            <?php
                // determinar la cantidad de filas que abarca el renglón de semestre
                $filasSemestre = 0;
                if (count($tiposMaterias) == 3) {
                    $filasSemestre += 2; //los dos reglones extra

                    foreach ($tiposMaterias as $tipo) {
                        $filasSemestre += count($tipo);
                    }
                } elseif (count($tiposMaterias) == 1) {
                    foreach ($tiposMaterias as $tipo) {
            ...
            <tr>
                ...
                <?php if (count($tiposMaterias) == 1) : ?>
                    ...
                    <?php foreach ($comunes as $codigo => $datos) : ?>
                        ...
                        <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                            <?php echo $codigo; //Código ?>
                        </td>
                        <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                            <?php echo $datos['nombre_materia']; //Materia ?>
                        </td>
                        <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                            <input type="checkbox" name=<?php echo $codigo; ?> value=<?php echo $codigo; ?> <?php echo ($datos['estado_materia'] == "Deshabilitada") ? "disabled" : ""; ?>>
                        </td>
                        <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                            <?php echo $datos['estado_materia']; //Estado ?>
                        </td>
                        <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                            <?php echo $datos['uc_materia']; //UC ?>
                        </td>
                        <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                            <?php echo formatoPrelacion($datos); //prelaciones materias y uc ?>
                        </td>
            </tr>
        ...
        <?php else : ?>
            ...
            <!-- Electiva Mención -->
            <td colspan=<?php echo $columnasTabla - 1; ?>> <strong> Electivas de la Mención </strong> </td>
            </tr>
            <tr>

                <?php $electivaMencion = $tiposMaterias['3']; //GestorMaterias::ELECTIVA_MENCION = "3";
                ?>

                <?php foreach ($electivaMencion as $codigo => $datos) : ?>
                    <!-- datos de cada materia en este renglón -->
                    <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                        <?php echo $codigo; //Código ?>
                    </td>
            ...
            <!-- Electiva Comunes -->
            <td colspan=<?php echo $columnasTabla - 1; ?>> <strong> Electivas Comunes </strong> </td>
            </tr>
            <tr>
                ...
                <?php foreach ($electivasComunes as $codigo => $datos) : ?>
                    ...
                    <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                        <input type="checkbox" name=<?php echo $codigo; ?> value=<?php echo $codigo; ?> <?php echo ($datos['estado_materia'] == "Deshabilitada") ? "disabled" : ""; ?>></td>
                    <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?>>
                        <?php echo $datos['estado_materia']; //Estado ?>
                    </td>
                    <?php if ($primero) : ?>
                        <td <?php $estilo = estiloEstado($datos['estado_materia']); echo $estilo; ?> rowspan=<?php echo count($electivasComunes) ?>><?php echo $datos['uc_materia']; //UC agrupadas
                                                                            ?></td>
                    <?php $primero = false;
                        endif; ?>
