...
class GestorAdmin extends Model
{
    // Constructor
    ...
    public function actualizarUsuario(string $id_, string $usuario_, string $cedula_, string $correo_, string $rol_, string $nombre_, string $apellido_, string $genero_, array $fechaNacimiento_) {
        ...
        $sql = "UPDATE usuarios 
                SET usuario = '{$usuario_}', cedula = '{$cedula_}', correo = '{$correo_}', rol = '{$rol_}', nombre = '{$nombre_}', apellido = '{$apellido_}', genero = '{$genero_}', fecha_nacimiento = '{$fechaNacimiento}'
                WHERE id = $id_";
    ...
    public function eliminarUsuario($id_) {
        ...
        if ($rol == '3') {
            ...
            $id_estudiante = $registroE['id'];                      // Campo id del Registro con el usuario Dado 
